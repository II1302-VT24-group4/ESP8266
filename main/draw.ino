...
void draw(const char *str) {
  ...
  do {
    ...
    int charPerLine = u8g2.getDisplayWidth() / u8g2.getMaxCharWidth();
    int lineSpacing = u8g2.getFontAscent() - u8g2.getFontDescent();

    for (int i = 0; i < strlen(str); i += charPerLine) {
      // Substring each line
      char line[charPerLine + 1];
      strncpy(line, &str[i], charPerLine);
      line[charPerLine] = '\0';

      // Draw each line
      u8g2.drawStr(0, 14 + (i / charPerLine) * lineSpacing, line);
    }
  } while (u8g2.nextPage());
}
...
/**
 * @brief Draws the status of the next room on the display.
 */
void drawNextRoom() {
  u8g2.firstPage();
  do {
    ...
    // Draw heading "Available rooms"
    const char* heading = "Available rooms:";
    int textWidth = u8g2.getStrWidth(heading);
    int xPosHeading = (128 - textWidth) / 2;
    u8g2.setCursor(xPosHeading, 15);
    u8g2.print(heading);

    // Find the maximum width of room names
    int maxWidth = 0;
    for (int j = 0; j < 5; j++) {
      if (roomsStatus[j]) {
        int lineWidth = u8g2.getStrWidth(nearestRoomsId[j].c_str());
        maxWidth = max(maxWidth, lineWidth);
      }
    }

    // Center each room name horizontally
    int yPos = 30;
    for (int j = 0; j < 5; j++) {
      if (roomsStatus[j]) {
        int lineWidth = u8g2.getStrWidth(nearestRoomsId[j].c_str());
        int xPosRoom = (128 - lineWidth) / 2;
        u8g2.setCursor(xPosRoom, yPos);
        u8g2.print(nearestRoomsId[j]);
        yPos += 15;
      }
    }
  } while (u8g2.nextPage());
